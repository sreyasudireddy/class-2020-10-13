...
# sample_n/rep_sample_n: work best on a tibble
...
```{r}
# make an empty tibble with the number of rows equal to the number of things that you want
random_samples <- tibble(replicate = 1:5) %>%
  
  # a list-column of 30 rows, each a sample form urn
...
```{r}
# a column for the # red beads
# a column for the # blue beads
# column is num_red > num_blue

probability_test <- tibble(replicate = 1:1000) %>%
    mutate(samp = map(replicate, ~ sample_n(urn, size = 50, replace = TRUE))) %>%
  mutate(is_red = map_dbl(samp, ~sum(.$color == "red")),
         is_blue = map_dbl(samp, ~sum(.$color == "blue")),
         red_greater = is_red > is_blue
        ) %>%
  summarize(probability = sum(red_greater)/n())
...
```{r}
rbinom(n = 1, size = 25, prob = probability_test$probability)
```
...
```{r}
 x <- tibble(numb_red_urn = seq(from = 100, to = 700, by = 100), 
       numb_red_paddle = map_int(numb_red_urn, ~ rbinom(n = 1, size = 25, prob = ./1000)))
x
```
...
```{r}
 x2 <- tibble(numb_red_urn = seq(from = 100, to = 700, by = 100), 
       numb_red_paddle = map(numb_red_urn, ~ rbinom(n = 10, size = 25, prob = ./1000))) %>%
